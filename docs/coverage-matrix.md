...
| ID | Concept | Tier | Feature that forces it | File / Export | Done |
|---|---|---|---|---|---|
| W2-D1-01 | `useEffect` — mount-only fetch (`[]` deps) | R | The `/underwriting` referral queue, loaded once on arrival | `src/routes/UnderwritingPage.tsx` — `UnderwritingPage` (`useFetch(async (signal) => Promise.all([…]), [])`), via `src/shared/hooks/useFetch.ts`. **Upgraded from `[~]` in Week 4 Day 5, and not by building something to carry it.** The row was qualified because no `[]`-deps *fetch* existed anywhere and the obvious place to add one — `PolicyClaimsTab` — was the one place it would have been a bug: Router reuses the tab instance across `:id` changes, so a literal `[]` there shows POL-1's claims forever. `/underwriting` has no route parameter and no filter; its three reads depend on nothing the component can change, so `[]` is the *correct* dependency list rather than a demonstration wedged in. The contrast is now one hook and two callers: `PolicyClaimsTab` passes `[policyId]` because it must, this page passes `[]` because it may. `[]`-deps effects that are not fetches were already solid: `useRenewalCountdown`'s interval (W2-D1-04) and `Modal`'s keydown listener | [x] |
...
| W3-D3-02 | `React.lazy` — component-level splitting | R | Claim intake wizard (heavy, dialog-only) | `src/routes/ClaimIntakePage.tsx` — module-scope `splitChunk('ClaimIntakeWizard', () => import('./claimIntake/ClaimIntakeWizard'))`, rendered only inside an open MUI `<Dialog>`. The container changed from the app's own `<Modal>` to `Dialog` in Week 4 Day 5 (the wizard became a MUI surface and took its chrome with it); the property the split depends on did not — `Dialog` renders nothing while `open` is false, exactly as `Modal` returned `null`, and passing `keepMounted` would download this chunk on page load while looking identical. The `import()` stays in this file (it is what draws the chunk boundary); only the memoise/log/reset mechanism is shared with the route chunks. Verified three ways: its own chunk in `vite build` (**50.41 kB**, up from 6.38 kB now that `react-hook-form` and the resolver ride in it, plus a shared 122 kB `schemas` chunk that is *also* out of the entry), *not* loaded while the dialog is closed by `src/App.test.tsx`, and the entry chunk measured before and after this pass at 634.18 → 637.09 kB | [x] |
...
| W4-D1-02 | MUI core components (`AppBar`, `Card`, `Table`, `Dialog`, `Button`, `TextField`) | R | `/dashboard` KPI cards, `/underwriting` table, `ClaimIntakeWizard` dialog | `src/shared/layout/AppShell.tsx` — `AppShell` (`AppBar` + `Toolbar` + `Drawer`, and the `TextField select` role switcher), `src/routes/DashboardPage.tsx` — `DashboardPage` (four metric `Card`s, the expiring-policies `Table`), and `src/routes/UnderwritingPage.tsx` — `UnderwritingPage` (four metric `Card`s and two `Table`s). **All six, as of Week 4 Day 5 — upgraded from `[~]`.** The two that were missing arrived with the surfaces that wanted them, which is what the qualification predicted: `Dialog` is `src/routes/ClaimIntakePage.tsx`'s, holding the rebuilt `ClaimIntakeWizard`; `Button` has three honest call sites — "Start a claim" on `/claims/new`, Back/Continue/Submit in the wizard's `DialogActions`, and "Record decision" in `src/routes/underwriting/DecisionForm.tsx`. None of them was placed for this row: the row waited for pages that needed them | [x] |
...
| W4-D2-04 | `styled()` API | R | A reusable themed component used on more than one MUI surface | `src/shared/theme/StatusChip.tsx` — `StatusChip` (`styled(Chip, { shouldForwardProp })` reading `theme.palette.status[status]`), rendered on /dashboard's book-by-status strip, in `/underwriting`'s referral queue table, and on the claim wizard's review step. The deliberate duplicate of `.statusActive`…`.statusCancelled` in `src/routes/policies/PolicyRow.module.css`, which stays; its header states what each of the two approaches costs. **Three MUI surfaces as of Week 4 Day 5 — upgraded from `[~]`,** and by exactly the route the qualification named: both new consumers want the badge for their own reasons. A referral queue that does not say whether a policy is pending or already lapsed is asking for a decision on incomplete information, and a review screen that does not show the status of the policy being claimed against hides the one fact that can invalidate the claim | [x] |
...
| W4-D5-01 | `useForm` + `register` + `Controller` | R | `ClaimIntakeWizard` fields — `register` for plain inputs, `Controller` for MUI controlled components | `src/routes/claimIntake/ClaimIntakeWizard.tsx` — `ClaimIntakeWizard` and `src/routes/underwriting/DecisionForm.tsx` — `DecisionForm`. Both mixes are deliberate and each `Controller` is there for a different, nameable reason rather than as a blanket policy: `Autocomplete` (the policy picker) has a ref, but it points at its *search* input, whose value is not the field's; MUI's `select` `TextField` renders a `Select` whose ref reaches a `div`; and the third-party `Checkbox`'s value is `checked`, not `value` — the wrong property on a ref that is otherwise fine. Everything with a real, readable native input is `register`ed: amount, description, location, phone, email, FIR number, rate loading, note. Each failure mode is silent in the same way — the field reads `undefined`, validation rejects a value the user can see on screen, and nothing in the console says why | [x] |
| W4-D5-02 | Zod schema + `zodResolver` + `z.infer` as the single source of type | R | Claim intake validation, with the form's TypeScript type derived from the schema rather than declared twice | `src/routes/claimIntake/claimSchema.ts` — `claimSchema` / `ClaimDraftValues` (`z.infer<typeof claimSchema>`), and `src/routes/underwriting/decisionSchema.ts` — `makeDecisionSchema` / `DecisionValues` (`z.infer<ReturnType<typeof makeDecisionSchema>>`). The hand-written `ClaimIntakeDraft` interface this replaced had already drifted — it declared `amount: number` while the input produced a string, bridged by a `Number(amount)` at the call site. Two schemas rather than one because they are two different *shapes* of cross-field rule: `claimSchema`'s `superRefine` compares two values inside the form (`amount` against the named policy's `sumInsured`), while `decisionSchema` is a **factory** whose cap comes from `GET /underwriting/limits` at run time — a rule that cannot be written until the request lands. Pinned by `src/routes/claimIntake/claimSchema.test.ts` (8 assertions, including that the cross-field issue carries `path: ['amount']`, which is what the per-step `trigger()` matches on) | [x] |
| W4-D5-03 | `formState` — `errors`, `isSubmitting`, `isDirty`, per-step `trigger()` | R | Wizard step gating — a step cannot be left until its own fields validate | `src/routes/claimIntake/ClaimIntakeWizard.tsx` — `goNext()` (`await trigger(STEP_FIELDS[step])`) with `mode: 'onBlur'` / `reValidateMode: 'onChange'`; the submit is `disabled={!isDirty \|\| isSubmitting}` in both this file and `DecisionForm`. `STEP_FIELDS` is a `Record<WizardStep, readonly (keyof ClaimDraftValues)[]>` so a renamed schema field breaks the build rather than producing a step gate that validates nothing and always passes. `isValid` is read but deliberately **not** used to disable Continue — a gate that cannot be pressed gives the agent no way to find out what is wrong — it drives the button's variant instead. Reachability and behaviour are pinned by `src/routes/claimIntake/ClaimIntakeWizard.test.tsx`: Continue on an empty Policy step surfaces the *schema's* message and does not advance | [x] |
| W4-D5-04 | Integrated authenticated dashboard (`/underwriting`) | R | The route that puts theming, axios, Redux Toolkit and the form stack together in one authenticated surface | `src/routes/UnderwritingPage.tsx` — `UnderwritingPage`. All four in one screen: MUI (`Grid2`, four `Card`s, two `Table`s, `StatusChip`, `Alert`, `sx`), axios (three parallel reads through every interceptor), Redux Toolkit (`RequireRole` gating on `state.auth.role`; the memoised `selectExposureByCustomer` for the totals), and react-hook-form (`DecisionForm`, on a schema built from the limits the server just sent). The parallelism is not incidental — it is what closes W4-D3-04. Reachability is pinned by `src/routes/UnderwritingPage.test.tsx`, which is the only test in the suite that renders this route at all: `App.test.tsx` walks the table as an `agent` and can therefore only assert the page is *absent* | [x] |
...
| C-06 | `useActionState` | C | Claim submit action with returned error state | `src/routes/policyDetail/PolicyClaimsTab.tsx` — `ClaimForm` (single-step form; the action reads `amount`/`description` straight from `FormData` and returns real per-field errors, rendered through `TextField`'s existing `error` prop). **Repointed in Week 4 Day 5: this row named two call sites and now names one.** `ClaimIntakeWizard` was the other, and the Week 4 form stack replaced its action with `handleSubmit` + a thunk. The one that survived is the more interesting of the two — validating out of `FormData` with per-field errors, rather than validating ahead and returning a single error slot — and it is untouched, unchanged and still reachable from `/policies/:id/claims`. Stays `[x]`: one real call site is what the row asks for, and it was never the wizard that carried it. | [x] |
| C-07 | `useFormStatus` (react-dom) | C | Submit button disabled/pending inside the form | `src/shared/components/SubmitButton.tsx` — `SubmitButton`, plus `FormCancelButton`, both inside `ClaimForm` in `src/routes/policyDetail/PolicyClaimsTab.tsx`. **Still `[~]`, and the qualification got worse in Week 4 Day 5 — this row regressed, and the regression is this pass's doing.** It used to read "one of the two call sites is unobservable": `ClaimIntakeWizard`'s was real, because `ClaimIntakePage` awaited the write before closing the modal, so the button rendered its pending state for the full 400–900ms round trip. That call site is gone. The wizard now submits through `handleSubmit` and reads `formState.isSubmitting`, and the swap was not optional — `useFormStatus().pending` is only ever true during a React **form action**, so a form driven by `onSubmit` reports `pending: false` forever no matter where the button sits. What remains is `ClaimForm`, whose pending state was already unobservable for an unrelated reason: `handleLogClaim` calls `modalRef.current?.close()` on its first line, before the `await`, so the whole `<form>` unmounts before `pending` can render. Not "fixed" by reordering — closing immediately is the correct UX there, because the optimistic row in the table below (C-05) *is* the feedback. So the hook is real, correct, reachable, and now observable **nowhere**. Recorded rather than papered over; see [Known weak claims](#known-weak-claims) | [~] |
...
**Total rows: 106. Solid `[x]`: 101. Qualified `[~]`: 3. Not built `[ ]`: 2.**

> Counted mechanically after Week 4 Day 5 rather than by hand — every row in
> the four concept tables that ends in a `[x]`, `[~]` or `[ ]` marker, which is
> 106, matching the by-section count below. The three `[~]` are W2-D2-06,
> W3-D1-02 and C-07; the two `[ ]` are W3-D4-04 and W3-D5-03. Every one of
> those five is argued in the table further down, and none of them is a Week 4
> row.
...
| C-07 | `useFormStatus` | **Worse than it was, and this repo did it.** The observable call site was `ClaimIntakeWizard`, whose submit is now `handleSubmit` + `formState.isSubmitting` (W4-D5-03). `useFormStatus().pending` is only true during a React *form action*, so it cannot follow the form there — it would report `false` forever. The one remaining call site is `ClaimForm`, which was already unobservable: `handleLogClaim` closes the modal before its `await`, unmounting the form before `pending` can render. Correct code, reachable code, observable nowhere. | A form in this app that still submits via `<form action={…}>` **and** stays mounted while the action runs. `ClaimForm` is the only form action left and it must unmount immediately, because the optimistic row (C-05) is the feedback and a lingering modal would cover it. Restoring the wizard's action would mean giving back the react-hook-form submit path the curriculum explicitly asks for. The two are mutually exclusive, and W4-D5 wins. |
...
**Four rows left this table in Week 4 Day 5**, and none of them by being argued
away. Each was qualified with a stated condition, and the condition was met by
a surface the app wanted anyway:

- **W2-D1-01** (`[]`-deps fetch) said it needed "a feature that genuinely
  fetches once and never refetches", and warned that forcing one into
  `PolicyClaimsTab` would ship a stale-data bug. `/underwriting` has no route
  parameter, so its three reads genuinely depend on nothing — `[]` is the
  correct list there, not a demonstration.
- **W4-D3-04** (single-flight 401) said it needed "`/underwriting`, which loads
  a queue, its filters and its totals together". That is what it does, in one
  `Promise.all`, and the burst is now reachable by clicking rather than only by
  running the test.
- **W4-D2-04** (`styled()` on more than one surface) said it needed "the
  /underwriting queue table rendering policy status, a surface that wants the
  badge for its own reasons". `StatusChip` now renders on three surfaces, and
  the review step's is the same argument: a decision screen that hides the
  status of the policy being decided on is hiding the fact that invalidates it.
- **W4-D1-02** (six MUI components, four built) said `Dialog` belonged to
  `ClaimIntakeWizard` and that a `Button` with no job would be "a component
  placed for this row". Both arrived with the pages that needed them.

The pattern is worth naming because it is the argument this file keeps making:
none of the four was fixed by editing the row. They were fixed by building the
thing the row was waiting for, and the waiting is what kept the earlier entries
honest.

**One row went the other way in the same pass, and it is this pass's fault:**
`C-07` (`useFormStatus`) was partially observable and is now observable
nowhere, because the wizard's form action — its one real call site — was
replaced by `react-hook-form`'s `handleSubmit`. That is a straight trade the
curriculum forces (W4-D5-03 asks for `isSubmitting`; `useFormStatus` cannot
read a form that has no action), and it is recorded as a loss rather than
rounded off. See the table above.
...
Total rows: **106**. Solid: **101**. Qualified: **3**. Not built: **2** (W3-D4-04 and W3-D5-03 — see [Still unchecked after Week 4 Day 5](#still-unchecked-after-week-4-day-5)). **Every Week 4 row is closed and solid.** The five rows that are not are all Week 2 / Week 3 / completeness-tier, and all five are argued in [Known weak claims](#known-weak-claims).
...
## Still unchecked after Week 4 Day 5

**Two rows, and neither is a Week 4 row.** This pass closed its four-row target
list — W4-D5-01 through W4-D5-04 — and four rows outside it, three of which
were `[~]` conditions this surface happened to satisfy and one of which was a
`[~]` from Week 2. It also **lost** ground on one row, which is recorded in the
same breath below rather than in a footnote.

### Closed

- **Target list (4):** W4-D5-01 (`useForm` + `register` + `Controller`),
  W4-D5-02 (Zod + `zodResolver` + `z.infer`), W4-D5-03 (`formState` + per-step
  `trigger()`), W4-D5-04 (the integrated `/underwriting`).
- **Upgraded from `[~]` (4):** W2-D1-01, W4-D1-02, W4-D2-04, W4-D3-04 — see
  [Repaired rather than downgraded](#repaired-rather-than-downgraded) for what
  each one's stated condition was and how it was met.
- **Repointed, still `[x]` (2):** W3-D3-02 (the wizard's container is a MUI
  `Dialog` now, and the chunk-split property survived the swap) and C-06 (the
  wizard's `useActionState` is gone; `PolicyClaimsTab`'s remains and was always
  the better of the two call sites).

### Lost

- **C-07 (`useFormStatus`) is now observable nowhere.** It was `[~]` on "one of
  two call sites"; the observable one was the wizard's, and it could not
  survive the move to `handleSubmit`, because `useFormStatus().pending` is only
  ever true inside a React form action. This is a real regression in evidence
  caused by a change the curriculum requires, and it is stated here, on the
  row, and in [Known weak claims](#known-weak-claims).

### Still open

- **W3-D4-04 (1):** `useLayoutEffect`. Unchanged and still honestly unbuilt —
  no screen in this app must read a rendered dimension before paint.
- **W3-D5-03 (1):** `use()` with Context, read conditionally. Unchanged — the
  Redux migration deleted the Context that carried it, and `useAppSelector`
  cannot be called inside a branch.
- **Three `[~]` (not counted above, since all three name code that exists):**
  W2-D2-06 (split context, inert), W3-D1-02 (`memo` comparator that can never
  run), C-07 (above).

### What this pass changed outside its own rows

**`ClaimIntakePage` became a MUI surface, and `src/shared/components/Modal.tsx`
did not die.** `CLAUDE.md`'s boundary table names `ClaimIntakeWizard` as a MUI
surface and does not list `/claims/new` on either side of the line. The same
document forbids putting MUI components inside a CSS Modules surface — and a
`DialogTitle`/`DialogContent`/`DialogActions` body cannot be hosted by a
hand-rolled `createPortal` modal without doing exactly that, so the container
followed the thing it contains: `Panel` → `Card`, `<button>` → `Button`,
`Modal` → `Dialog`, `Toast` → `Snackbar`. `Modal` and `Toast` both keep their
consumer in `PolicyClaimsTab`, so W3-D4-05 (`forwardRef`), W3-D4-06
(`useImperativeHandle`) and C-09 (`createPortal`) are untouched. `/policies`,
`/policies/:id` and `/quote` are untouched and remain the control group.

**A second Redux slice.** `src/shared/store/claimsSlice.ts` holds
`submitClaimThunk` and the list of what this session filed. It exists because
the brief asks the wizard's submit to "post through the axios client and
dispatch to the store", and a thunk is the one construct that is both. It is
not folded into `auth` for a stated reason: `logout` writes every field of
`AuthState` out by hand precisely so nothing survives it, and a combined slice
would put the agent's own record of what they filed inside that blast radius.

**Four new endpoints and three new mock routes.** `GET /claims`,
`GET /underwriting/limits` and `POST /underwriting/decisions`, plus a wrapper
for the already-existing `GET /policies`. `endpoints.ts` had said, in writing,
that it held exactly two functions and would grow when `/underwriting` gave the
other routes a real consumer; that is what happened, and the three mock routes
with still no component calling them still have no wrapper.

The limits endpoint is worth one line on its own: it reads the role out of the
**presented bearer token** rather than out of a query parameter, which makes it
the only place in the app where the request interceptor's work is visible in a
response body.

**One dependency decision, stated rather than buried.** The brief asks for
`Controller` on "the MUI Select and DatePicker". `@mui/x-date-pickers` is not
in `CLAUDE.md`'s pinned list, and that document says "nothing else without
asking", so it was not installed. The date control is a MUI
`TextField type="date"` in the picker's role. It is still on a `Controller`,
and the honest reason is *not* the one that applies to the two selects above
it — a native date input has a usable ref, so `register` would have worked.
It is wired this way so that swapping in a real `DatePicker` the day that
dependency is approved is a one-element change. That is a small amount of
machinery bought for a small blast radius, and `ClaimIntakeWizard.tsx`'s header
says so rather than leaving it to be found.

**Measured, not assumed — the bundle.** `vite build` before and after this
pass, same machine:

| Chunk | Before | After |
|---|---|---|
| entry `index-*.js` | 634.18 kB (207.14 kB gzip) | 637.09 kB (208.14 kB gzip) |
| `ClaimIntakeWizard` | 6.38 kB | 50.41 kB |
| shared `schemas-*.js` (Zod) | — | 122.06 kB |
| `ClaimIntakePage` | 1.74 kB | 14.32 kB |
| `UnderwritingPage` | 1.81 kB | 8.22 kB |
| `DashboardPage` | 21.83 kB | 6.19 kB |

The number that matters is the first: **the entry grew by 2.91 kB while the app
gained `react-hook-form`, `zod` and `@hookform/resolvers`.** All three are
reachable only from lazy chunks, so Rollup put Zod in its own shared chunk that
two lazy routes pull in and the entry does not. That is the code-splitting
claim (W3-D3-01) doing real work for the first time since Week 4 Day 2 blunted
it — and it is the exact opposite of what happened when MUI arrived, because
MUI's theme provider is a static import in `main.tsx` and these libraries are
not imported by anything above the router.

`DashboardPage` getting *smaller* is the same mechanism read the other way, and
it is not a saving: `Table`, `Card`, `Link` and `LinearProgress` were hoisted
out of it into shared chunks (`TableRow` 6.82 kB, `StatusChip` 14.96 kB,
`LinearProgress` 21.08 kB, `Link` 4.00 kB) because `/underwriting` now needs
them too. The bytes moved; the dashboard still downloads them.
